...
void quote_file::write_to_json_file (FILE *fp)
{
    ...
    fprintf(fp, "  \"MrEnclaveHex\": \"%s\",\n", format_hex_buffer(hex_buffer, hex_buffer_size, _parsedReport.identity.unique_id, OE_UNIQUE_ID_SIZE));
    fprintf(fp, "  \"MrSignerHex\": \"%s\",\n", format_hex_buffer(hex_buffer, hex_buffer_size, _parsedReport.identity.signer_id, OE_SIGNER_ID_SIZE));
    fprintf(fp, "  \"ProductIdHex\": \"%s\",\n", format_hex_buffer(hex_buffer, hex_buffer_size, _parsedReport.identity.product_id, OE_PRODUCT_ID_SIZE));
    ...
    fprintf(fp, "  \"QuoteHex\": \"%s\",\n", format_hex_buffer(hex_buffer, hex_buffer_size, _quote, _quote_size));
    fprintf(fp, "  \"EnclaveHeldDataHex\": \"%s\"\n", format_hex_buffer(hex_buffer, hex_buffer_size, _enclave_held_data, _enclave_held_data_size));
    fprintf(fp, "%s\n", "}");
}
